...
class BRNN(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        self.sequence = RNN(input_dim = self.input_dim, act_dim = self.act_dim, time_steps = self.time_steps,
                            output_dim = self.output_dim, unit_output_dim = self.unit_output_dim, return_sequences = True, 
                            act = self.act, acty = self.acty)
        
        self.reverse_sequence = RNN(input_dim = self.input_dim, act_dim = self.act_dim, time_steps = self.time_steps, 
                                    output_dim = self.output_dim, unit_output_dim = self.unit_output_dim, return_sequences = True, 
                                    act = self.act, acty = self.acty)
    ...
    def forward(self, X):
        ...
        A1 = self.sequence(X)  
        A2 = self.reverse_sequence(torch.flip(X, dims = [1]).to(self.device))
